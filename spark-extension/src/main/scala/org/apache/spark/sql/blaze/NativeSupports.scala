...
case class BlazeCallNativeWrapper(
  ...
  def finish(): Unit = {
    if (!isFinished) {
      finished.set(true)
      logInfo(s"releaseRuntime")
      JniBridge.releaseRuntime(runtimePtr)
    }
  }
  ...
  protected def enqueueWithTimeout(value: Object): Boolean = {
    valueQueue.offer(value, 10, TimeUnit.MILLISECONDS)
  }

  protected def dequeueWithTimeout(): Object = {
    valueQueue.poll(10, TimeUnit.MILLISECONDS)
  }

  protected def enqueueError(value: Object): Boolean = {
    errorQueue.offer(value, 10, TimeUnit.MILLISECONDS)
  }
